...
def load_lobster_data(
    ...
    # segment id: constant reference price
    df["seg_id"] = df["pref"].ne(df["pref"].shift(1)).cumsum()

    # per-queue inter-event time within each segment
    df.insert(1, "dtk_l", df.groupby(["seg_id", "lvl"])["time"].diff())

    # fill first event of each (seg_id, lvl) with time since segment start
    seg_start = df.groupby("seg_id")["time"].transform("min")
    first = df["dtk_l"].isna()
    df.loc[first, "dtk_l"] = df.loc[first, "time"] - seg_start[first]
    df.loc[df.index[0], "dtk_l"] = (df.loc[df.index[0], "time"] - market_open_seconds)
    
    df.insert(1, "dtk", df["time"].diff())
    df.loc[df.index[0], "dtk"] = (df.loc[df.index[0], "time"] - market_open_seconds)
...
def compute_ait_by_level(df: pd.DataFrame, K: int, lvl_col: str = "lvl", time_col: str = "dtk_l") -> pd.Series:
    # AIT_i = mean dtk of all events at |lvl|=i (L/C/M included)
    abs_lvl = df[lvl_col].abs()
