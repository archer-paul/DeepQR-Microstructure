...
def load_lobster_data(
    ...
    # --- AJOUT ICI : Calcul du spread en nombre de ticks ---
    ...
    # base df = msg + ob
    # --- MODIFICATION ICI : On ajoute pd.Series(spread_ticks) dans le DataFrame final ---
    df = pd.concat([
        msg[["time","type","price", "lvl", "size"]], 
        pd.Series(spread_ticks, name="spread", index=msg.index),  # <--- Le spread est injecté là
        qr.drop("tick_size", axis=1)
    ], axis=1)
