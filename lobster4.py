...
def load_lobster_data(
    ...
    # base df = msg + qr + ob (RAW ORDERBOOK INCLUS ICI)
    # L'ajout de `ob` ici garantit que 'bid_px_1', etc., survivent jusqu'au Notebook
    df = pd.concat([
        msg[["time", "type", "price", "lvl", "size"]], 
        qr.drop("tick_size", axis=1, errors='ignore'), 
        ob
    ], axis=1)
    
    # On nettoie les événements hors de la grille
    df = df[df.lvl != 0]
